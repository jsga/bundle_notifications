...
def test_load_data():
    ...
    assert isinstance(
        df, pd.core.frame.DataFrame), "Loaded data is not a pandas DataFrame"
...
def test_create_message_single():
    ...
    assert bundle_notifications.create_message_single(
        1, 'Javi') == 'Javi went on a tour'

    assert bundle_notifications.create_message_single(
        2, 'Javi') == 'Javi and 1 other went on a tour'

    assert bundle_notifications.create_message_single(
        3, 'Javi') == 'Javi and 2 others went on a tour'
...
def test_bundle_func():
    ...
    d_fake = {'timestamp': {0: pd.Timestamp('2017-08-01 00:06:47'),
              ...
              'friend_name': {0: 'Geir', 1: 'Antim', 2: 'Σωτήριος',
                              3: 'Mona', 4: 'Laura'}}
    ...
    assert isinstance(
        ...
        "Output element is not a pandas DataFrame"

    assert df_test.shape == (5, 5), 'Output dimensions mismatch.'

    assert np.all(df_test.columns == ['notification_sent',
                                      'timestamp_first_tour',
                                      'tours', 'receiver_id', 'message']), \
        "Wrong output column names"
