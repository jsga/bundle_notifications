...
from .optimal_delay import delay, local_search
...
def add_notif_counter(x):
    ...
    ----------
    x : np.array of int
        array of length 4 with. Each element is an index, indicating the timestamp when the notification should be sent. Example: x = np.array([0,1,2,5])
...
def count_tours_per_notif(notification_counter, friend_id, friend_name,timestamp):
    """

    Equivalent to df_g['tours'] =  df_g.groupby('notification_counter')['friend_id'].apply(lambda x: (1- x.duplicated()).cumsum())

    In pseudo-code, this is equivalent to:

    - As an input, we have a dataset filtered by user_id and day of the year
    - Each of the inputs of this functions are numpy arrays, corrsponding to a column of the dataset. Doing basic in numpy is much faster, especially if we manage to use a @jit compilator (TODO)
    - Let us call the solution _tours_. For each element in friend_id we do: 
        ...
        - We continue until i in notification_counter
            - Reset tours = 1
            - Keep track of the name and timestamp of the first element  (name_first,timestamp_first_tour)


    Parameters
    ----------
    notification_counter : np.array
        notification counter. Could be the output of an optimal_delay method. For example, np.array([1,2,3,10]) for a 10 element array
    ...
    tours : np.array of int
        Count of the number of tours done since the last notification was sent, for unique friends-id
    name_first : np.array of <U256
        Name of the friend who first did a tour since the last notification was sent. len(name_first) <=4
    timestamp_first_tour : np.array of datetime64[ns]
        Timestamp of the first tour done by a friend, since the last notification was sent. len(timestamp_first_tour) <=4
    message : np.array of np.array of <U256
        Message to be sent. len(message) <= 4
    """
    ...
    for i in range(0, N):
        ...
        if prev_counter == notification_counter[i]:
            ...
            # Create message
            message[prev_counter-1] = create_message_single(tours_counter,name_first[prev_counter-1])
...
def bundle_func(df_g):
    ...
    This function is meant to be used after a pandas groupping (or manual filtering) of user_ids. 
    ...
    -------
    pd.DataFrame
        DataFrame containing 5 extra columns: ['notification_bool','tours','notification_counter','message','timestamp_first_tour']
    """
    ...
    if df_g.shape[0] <= 4:
        ...
        df_g['tours'] = 1        
        df_g['notification_counter'] = list(range(1,df_g.shape[0]+1)) # [1,2,3,4]
        ...
        # We also get the names and timestamp of the first element in the notif counter
        df_g['tours'], name_first,timestamp_first_tour, message = count_tours_per_notif(df_g.notification_counter.to_numpy('int'),df_g.friend_id.to_numpy(),df_g.friend_name.to_numpy(),df_g.timestamp.to_numpy())
...
def bundle(df):
    ...
    -------
    pd.DataFrame
        Contains 4 derived columns: ['notification_sent', 'timestamp_first_tour', 'tours', 'receiver_id', 'message']
    """
    ...
    df.rename(columns = {'timestamp':'notification_sent','user_id':'receiver_id'}, inplace=True)
    
    # END
    return df[['notification_sent', 'timestamp_first_tour', 'tours','receiver_id', 'message']]